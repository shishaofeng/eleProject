<template>
    <div ref="tableWarp" class="table">
        <HxTable ref="table" size="mini" style="height: 100%" :data="data" v-bind="tableProps" v-on="$listeners">
            <el-table-column v-if="showSelectionCol" type="selection" align="center" width="55"></el-table-column>

            <el-table-column v-if="showIndexCol" type="index" align="center" width="56" label="序号"></el-table-column>

            <slot></slot>

            <template v-if="empty" v-slot:empty>
                <img style="margin-top: 30px" src="@/assets/image/empty.png" alt="" />
            </template>

            <el-table-column v-for="(item, index) in customColumns" :key="item.key || index" v-bind="item">
...
<script>
// import ElTable from './el-table/table.vue'
...
export default {
    ...
    mounted() {
        const container = this.$refs.tableWarp.getElementsByClassName('el-table__body-wrapper')[0]
    ...
    methods: {
        ...
        load() {
            if (this.limitData?.length < this.data?.length) {
                this.limitData = this.limitData.concat(this.data.slice(this.count, this.count + 3))
                this.count += 3
            }
        },
    },
